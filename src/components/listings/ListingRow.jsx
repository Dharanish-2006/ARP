...
// Combines the standard home fields (beds/baths/etc.) with any Custom
// Fields the admin added on this listing (Lease, Monthly Rent, Annual
// Revenues, Annual Adj. Net, Square Feet, etc. for business listings).
...
export default function ListingRow({ property }) {
  ...
  return (
    <div className="flex flex-col overflow-hidden rounded-lg border border-edge bg-white transition-shadow duration-200 hover:shadow-lg sm:flex-row">
      <Link
        to={detailHref}
        className="block w-full shrink-0 overflow-hidden bg-surface-alt sm:h-auto sm:w-[300px]"
      >
        {property.images?.[0] ? (
          <img
            src={property.images[0]}
            alt={property.title}
            className="h-auto w-full object-contain transition-transform duration-300 hover:scale-105 sm:h-full sm:object-cover"
          />
        ) : (
          <div className="flex h-56 w-full items-center justify-center text-content-muted sm:h-full">
      ...
      <div className="flex flex-1 flex-col justify-between gap-5 p-5 sm:flex-row sm:p-6">
        <div className="min-w-0 flex-1">
          <Link
            to={detailHref}
            className="block font-display text-xl leading-snug text-[#12211d] transition-colors hover:text-[#1a5aa0] sm:text-2xl"
          >
            {property.title}
          </Link>
            ...
            <dl className="mt-4 grid grid-cols-2 gap-x-6 gap-y-2 border-t border-edge pt-4 text-sm sm:grid-cols-3">
              {specs.map((s) => (
                <div key={s.label} className="flex items-baseline justify-between gap-2 sm:block">
                  <dt className="text-[#4a4a4a]">{s.label}:</dt>
                  <dd className="font-semibold text-[#1c231f] sm:mt-0.5">{s.value}</dd>
                </div>
              ))}
            </dl>
          )}
        ...
        <div className="flex shrink-0 flex-row items-center justify-between gap-3 border-t border-edge pt-4 sm:w-44 sm:flex-col sm:items-end sm:border-t-0 sm:border-l sm:pt-0 sm:pl-6 sm:text-right">
          <div className="flex flex-col gap-2 sm:items-end">
            <p className="font-mono text-xl font-semibold text-[#1c231f]">{formatPrice(property)}</p>
          </div>
          <Link
            to={detailHref}
            className="inline-flex items-center justify-center rounded-full bg-brass-500 px-5 py-2.5 text-xs font-semibold uppercase tracking-wide text-[#1c231f] transition-colors hover:bg-brass-400"
          >
            View Details
          </Link>
        </div>
